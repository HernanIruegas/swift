...
class RiderViewController: UIViewController, MKMapViewDelegate, CLLocationManagerDelegate {
    ...
    var driverOnTheWay = false//helpful so that we don't update the table twice at the same time
   ...
   override func prepare(for segue: UIStoryboardSegue, sender: Any?) {
    ...
    func locationManager(_ manager: CLLocationManager, didUpdateLocations locations: [CLLocation]) {
        ...
        if let location = manager.location?.coordinate {
            ...
            if driverOnTheWay == false {//if your uber request has not been accepted by a driver
        ...
        //we check that, when the location is updated, wether or not the request has been accepted by a driver
            ...
            query.findObjectsInBackground(block: { (objects, error) in
                ...
                if let riderRequests = objects {
                    ...
                    for riderRequest in riderRequests {
                        ...
                        //now that we have confirmed that there is a rider's request
                        //we want to check if someone has responded to it
                        
                        if let driverUsername = riderRequest["driverResponded"] {//then we have a driver that has responded
                            
                            //now we want to found the driver's lcoation
                                ...
                                if let driverLocations = objects {
                                    ...
                                    for driverLocationObject in driverLocations {
                                        ...
                                        if let driverLocation = driverLocationObject["location"] as? PFGeoPoint {
                                            ...
                                            self.driverOnTheWay = true//your uber request has been accepted by a driver
                                            
                                            //to find the distance between rider and driver, make a CLLocation out of both locations, and then apply math
                                            ...
                                            //we show the distance between rider and driver
                                            self.callAnUberButton.setTitle("Your driver is \(roundedDistance)km away!", for: [])
                                            
                                            //now we want both locations (driver and rider) to appear on the mapView
                                            
                                            //change in latitude between one side of the map and the other
                                            //this change needs to cover the distance between rider and driver if we want both to appear on screen
